...
// ── Eager-loaded (above the fold on home) ────────────────────────────────────
import Onboarding from './Onboarding';
import IngestFlow from './IngestFlow';
import Chat from './Chat';

// ── Lazy-loaded (split into separate bundles) ─────────────────────────────────
...
// ── NavBar — memoised so it doesn't re-render on Redux state changes ──────────
    ...
    return (
        <nav className="pulse-nav">
            ...
            <div className="flex items-center gap-3 text-sm text-neutral-500">
                <a href="https://github.com/herin7" target="_blank" rel="noreferrer"
                ...
                <a href="mailto:herinsoni3737@gmail.com"
...
function HomeApp() {
    ...
    useEffect(() => {
        ...
        fetch('/api/session', { credentials: 'include' })
            .then((r) => r.json())
            .then((data) => {
                if (data.exists) localStorage.setItem('lc_user_id', data.userId);
        ...
        <BrowserRouter>
            ...
            <Routes>
                ...
                <Route path="/how-it-works" element={
                    <Suspense fallback={<PageShell />}><HowItWorks /></Suspense>
                } />
                <Route path="/hire-me" element={
                    <Suspense fallback={<PageShell />}><HireMe /></Suspense>
                } />
                <Route path="/byok" element={
                    <Suspense fallback={<PageShell />}><BYOKPage /></Suspense>
                } />
            </Routes>
        </BrowserRouter>
    );
